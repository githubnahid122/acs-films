    ...
    <section className="py-20 sm:py-28 bg-white">
      <div className="container mx-auto px-4 sm:px-6">
        <SectionHeader
          ...
          title="A Glimpse of MCCC"
          subtitle="From casting sessions to workshops and international festivals — life at MCCC."
          className="mb-12"
        />
          ...
          {items.map((img, i) => {
            ...
            return (
              <motion.div
                ...
                className={`group relative overflow-hidden rounded-xl cursor-pointer ${isWide ? 'md:col-span-2' : ''} ${isTall ? 'row-span-2' : ''}`}
              >
                <img
                  src={img.src}
                  alt={img.alt}
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-[#0F172A]/70 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col justify-end p-4">
                  <span className="text-xs font-medium text-violet-300 uppercase tracking-wide">{img.category}</span>
